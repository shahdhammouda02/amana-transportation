...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Header */}
      <header className="bg-green-500 text-white p-4 text-center font-bold text-xl">
        {busData.company_info.name} <br />
        <span className="text-sm font-normal">
          Proudly serving Malaysian Bus Riders since {busData.company_info.founded}
        </span>
      </header>
        ...
        {buses.map((bus, index) => (
          <button
            ...
            className={`px-4 py-2 rounded ${
              activeBus?.id === bus.id ? "bg-green-600 text-white" : "bg-white border"
            }`}
      ...
      {/* Map */}
      {activeBus && L && (
        <MapContainer center={[activeBus.current_location.latitude, activeBus.current_location.longitude]} zoom={12} style={{ height: "400px", width: "100%" }}>
          <TileLayer
          ...
          <Polyline
            positions={activeBus.bus_stops.map((stop) => [stop.latitude, stop.longitude])}
            color={
              activeBus.id === 1
                ? "red"
                : activeBus.id === 2
          ...
          {activeBus.bus_stops.map((stop) => (
            ...
            >
              <Popup>
                ...
                {stop.is_next_stop && <span className="text-red-500 font-bold"> (Next Stop)</span>}
              </Popup>
            </Marker>
          ))}
        </MapContainer>
